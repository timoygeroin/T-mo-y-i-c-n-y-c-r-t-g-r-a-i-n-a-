...
test("accepts only live-head-bound direct status while preserving stale and summary surfaces as non-authority", () => {
  const verdict = compileLiveStatusAuthority(
    input({
      evidence: [
  ...
  assert.equal(verdict.action, "accept_live_status_evidence");
  assert.deepEqual(verdict.accepted_surface_ids, ["current-check"]);
  assert.deepEqual(verdict.stale_surface_ids, ["repaired-head-check"]);
  assert.deepEqual(verdict.summary_surface_ids, ["pr-body-status-summary"]);
  assert.deepEqual(verdict.warnings, ["Node.js 20 Actions deprecation notice"]);
});

test("blocks prompt and PR-body summaries from impersonating current status", () => {
  const verdict = compileLiveStatusAuthority(
    input({
      evidence: [
        surface({
  ...
  assert.equal(verdict.action, "block_summary_as_status");
  assert.match(verdict.blockers.join("; "), /summary surface cannot prove live-head status/);
});

test("blocks stale direct status surfaces that belong to an older head", () => {
  const verdict = compileLiveStatusAuthority(
    input({
      evidence: [
        surface({
  ...
  assert.equal(verdict.ok, false);
  assert.equal(verdict.action, "block_stale_status_evidence");
});

test("routes live-head failures to repair instead of status acceptance", () => {
  const verdict = compileLiveStatusAuthority(
    input({
      evidence: [
        surface({
  ...
  assert.deepEqual(verdict.blockers, ["Run proof examples exited with 1"]);
});

test("holds pending live-head status without inventing a pass/fail claim", () => {
  const verdict = compileLiveStatusAuthority(input({ evidence: [surface({ verdict: "pending" })] }));

  assert.equal(verdict.ok, false);
  assert.equal(verdict.action, "hold_for_live_status");
});
