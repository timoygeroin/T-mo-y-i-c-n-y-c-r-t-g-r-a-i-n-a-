...
const admitted = evaluateEmbodimentNovelty({ candidate: candidate(), spent_families: spentFamilies });
assert.equal(admitted.ok, true);
assert.equal(admitted.action, "admit_novel_embodiment");
assert.ok(admitted.novel_axes.includes("artifact_class"));
assert.ok(admitted.novel_axes.includes("behavior_surface"));
assert.ok(admitted.decisive_evidence.includes("evaluateEmbodimentNovelty"));

const repeated = evaluateEmbodimentNovelty({
  candidate: candidate({
    artifact_class: "terminal_release_mux",
    execution_phase: "terminal_release_selection",
    behavior_surfaces: ["single terminal release admission"],
...
assert.equal(repeated.action, "block_repeated_family");
assert.deepEqual(repeated.repeated_family_ids, ["finalization-release-mux"]);

const proofOnly = evaluateEmbodimentNovelty({
  candidate: candidate({
...
assert.equal(proofOnly.action, "block_no_behavior_file");

const thinNovelty = evaluateEmbodimentNovelty({
  candidate: candidate({
    artifact_class: "head_surface_arbitration",
    execution_phase: "surface_intake",
    behavior_surfaces: ["live PR metadata arbitration"],
...
assert.equal(thinNovelty.ok, false);
assert.equal(thinNovelty.action, "block_insufficient_novelty");

console.log("embodiment novelty vector proof passed");
