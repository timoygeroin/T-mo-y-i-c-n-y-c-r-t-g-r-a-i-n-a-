...
function liveReceptors(transport) {
  return [
    { name: transport === 'vercel_ai_gateway' ? 'openai-model-via-vercel-gateway' : 'openai-model-direct', kind: 'domain_native', execute: true, live: true, verified: true },
    { name: 'local-host-state', kind: 'conversation', read: true, write: true, live: true, verified: true }
  ];
}

function boundedText(value) { return typeof value === 'string' ? value.slice(0, MAX_HISTORY_TEXT) : ''; }
...
function gatewayModel(model) { const v = String(model || DEFAULT_MODEL).trim(); return v.includes('/') ? v : `openai/${v}`; }
function directOpenAIModel(model) { const v = String(model || DEFAULT_MODEL).trim(); return v.startsWith('openai/') ? v.slice(7) : v; }

export function resolveGatewayToken(explicitToken) {
  return explicitToken || process.env.AI_GATEWAY_API_KEY || process.env.VERCEL_OIDC_TOKEN || undefined;
}

export function resolveModelTransport({ gatewayToken, apiKey = process.env.OPENAI_API_KEY, model = DEFAULT_MODEL } = {}) {
  if (gatewayToken) return { transport: 'vercel_ai_gateway', provider: 'OpenAI via Vercel AI Gateway', endpoint: AI_GATEWAY_RESPONSES_URL, token: gatewayToken, model: gatewayModel(model), auth_source: process.env.AI_GATEWAY_API_KEY && gatewayToken === process.env.AI_GATEWAY_API_KEY ? 'ai_gateway_api_key' : 'vercel_oidc_env' };
...
export async function callOpenAI({ apiKey = process.env.OPENAI_API_KEY, gatewayToken, model = DEFAULT_MODEL, reasoningEffort = DEFAULT_REASONING_EFFORT, message, context = {}, fetchImpl = fetch }) {
  ...
  const response = await fetchImpl(transport.endpoint, {
    ...
    body: JSON.stringify({ model: transport.model, store: false, reasoning: { effort: reasoningEffort }, instructions: developerInstructions(move), input: buildModelInput(message, context) }),
    signal: AbortSignal.timeout(120_000)
  });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) { const error = new Error(payload?.error?.message || `${transport.provider} Responses API returned HTTP ${response.status}`); error.status = response.status; throw error; }
...
export async function handleRuntimeRequest(req, res, options = {}) {
  if (req.method === 'GET' && req.url === '/api/organism/health') {
    ...
    return json(res, 200, { ok: true, kernel: 'mondayid-organism-kernel', model: options.model || process.env.MONDAYID_MODEL || DEFAULT_MODEL, model_transport_available: gatewayAvailable || directOpenAIAvailable, preferred_transport: gatewayAvailable ? 'vercel_ai_gateway' : (directOpenAIAvailable ? 'direct_openai' : null) });
  }
  ...
  try {
    ...
    const result = await callOpenAI({ apiKey: options.apiKey ?? process.env.OPENAI_API_KEY, gatewayToken: options.gatewayToken, model: options.model || process.env.MONDAYID_MODEL || DEFAULT_MODEL, reasoningEffort: options.reasoningEffort || process.env.MONDAYID_REASONING || DEFAULT_REASONING_EFFORT, message: body.message, context: body.context || {}, fetchImpl: options.fetchImpl || fetch });
