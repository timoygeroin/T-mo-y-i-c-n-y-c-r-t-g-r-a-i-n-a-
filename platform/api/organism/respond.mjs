...
function bodyBytes(body) { try { return Buffer.byteLength(JSON.stringify(body ?? {})); } catch { return MAX_BODY_BYTES + 1; } }
...
export default async function handler(req, res) {
  ...
  } catch (error) { return send(res, Number(error?.status) || 500, { ok: false, error: error?.message || 'RUNTIME_ERROR' }); }
}
